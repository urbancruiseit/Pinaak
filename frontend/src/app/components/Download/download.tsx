...
import { getDownloadReportThunk, resetDownloadState } from "@/app/features/Download/downloadSlice";
...
const Download = () => {
  ...
  // Reset state when user leaves the page
  useEffect(() => {
    return () => {
  ...
  // Reset local state when report type changes
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (result.payload?.length === 0) {
        alert(`No ${reportType} found for selected period`);
      }
    } catch (error) {
  ...
  const downloadCSV = () => {
    if (!reportData?.length) {
      alert("No data to download. Please click Show first.");
      return;
    }
      ...
      <div className="rounded-xl border bg-white shadow-sm p-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Select a report type, filter by period, and export as CSV</p>
          </div>
        </div>
            ...
            {loading ? (
              ...
              <>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3.375 19.5h17.25m-17.25 0a1.125 1.125 0 0 1-1.125-1.125M3.375 19.5h1.5C5.496 19.5 6 18.996 6 18.375m-3.75.125V6.375A2.25 2.25 0 0 1 4.5 4.125h15A2.25 2.25 0 0 1 21.75 6.375v12M6 18.375V6.75m0 11.625h12.75M6 6.75h12.75" />
                </svg>
                Show Report
              </>
            )}
