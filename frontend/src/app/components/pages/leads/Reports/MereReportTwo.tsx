...
export default function MonthlyEnquiryReportUI() {
  ...
  // Current month + next 4 months
  const getFiveMonths = (startIndex) => {
    return Array.from({ length: 5 }, (_, i) => {
      return months[(startIndex + i) % 12];
    });
  };

  return (
    <div className="p-4 overflow-auto bg-white">
      {months.map((currentMonth, sectionIndex) => {
        const displayMonths = getFiveMonths(sectionIndex);
